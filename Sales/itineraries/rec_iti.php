...
<html lang="en" dir="ltr">
  <head>
    <meta charset="utf-8">
    <title>Recieved Itineraries</title>
  ...
  <body>
    <div id = 'top_box' class='container-fluid' style = 'box-shadow:0px 5px grey; width:100%;'>
      <a href="../sales_dash.php"><img src="logonew.png" alt="Company Logo" id = 'logo'></a>
      <p style = 'float:right; color:white; display:inline; margin-top:1.5%;'><strong>Recieved Itineraries</strong></p>
    </div>
    <br>
    <div class="container-fluid">
      <h3>Requested Itineraries</h3>
      <small>Hey there, <?php echo $d_name ?>! These are all the <strong>recieved</strong> requests on the shelf!</small>
      <div class="container-fluid">
        <?php
          switch ($m_type){
            case "salesmanager":
            ...
            if($res_sp->num_rows){
              while($row_sp = $res_sp->fetch_assoc()){
                $sp_sno = $row_sp["sno"];
                $sql_sp_forms = "SELECT * FROM agent_form_data WHERE sales_partner_name = '".$sp_sno."' AND formstatus = 'submitted' ORDER BY ref_num DESC";
            ...
            if($res_sm->num_rows){
              if($row_sm = $res_sm->fetch_assoc()){
                $hp_id = $row_sm["hp_sno"];
                $sql_hp_forms = "SELECT * FROM agent_form_data WHERE sales_partner_name = '".$hp_id."' AND formstatus = 'submitted' ORDER BY ref_num DESC";
                ...
                if($row_sm = $res_sm->fetch_assoc()){
                  ...
                  if($res_hp->num_rows){
                          if($row_hp = $res_hp->fetch_assoc()){
                            ...
                            $sql_sup_forms = "SELECT * FROM agent_form_data WHERE sales_partner_name = '".$sup_id."' AND formstatus = 'submitted' ORDER BY ref_num DESC";
            ...
            $sql_rm = "SELECT * FROM agent_form_data WHERE formstatus = 'submitted' ORDER BY ref_num DESC";
